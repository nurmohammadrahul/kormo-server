...
const uri = `mongodb+srv://${process.env.DB_USER}:${process.env.DB_PASS}@cluster0.z6dqc.mongodb.net/?retryWrites=true&w=majority&appName=Cluster0`;
...
async function run() {
  try {
    const jobsCollection = client.db('kormo').collection('jobs')
    const bidsCollection = client.db('kormo').collection('bids')
